...
public class BTreeFile implements DbFile {
  ...
  public Page readPage(PageId pid) {
    ...
    try (BufferedInputStream bis = new BufferedInputStream(new FileInputStream(f))) {
      if (id.pgcateg() == BTreePageId.ROOT_PTR) {
        ...
        if (bis.skip(BTreeRootPtrPage.getPageSize() + (long) (id.pageNumber() - 1) * BufferPool
            .getPageSize())
            != BTreeRootPtrPage.getPageSize() + (long) (id.pageNumber() - 1) * BufferPool
            .getPageSize()) {
  ...
  public void writePage(Page page) throws IOException {
    ...
    if (id.pgcateg() != BTreePageId.ROOT_PTR) {
      rf.seek(BTreeRootPtrPage.getPageSize() + (long) (page.getId().pageNumber() - 1) * BufferPool
          .getPageSize());
    }
    rf.write(data);
    rf.close();
  ...
  protected BTreeLeafPage splitLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages,
    ...
    // Step 9. Return the desired page.
  ...
  protected BTreeInternalPage splitInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages,
    ...
    // Step 2. Get the middle entry key which would be pushed up to the parent.
    assert rIt.hasNext();
    BTreeEntry midEntry = rIt.next();
    // Step 3. Create the new page and mark it as dirty.
    ...
    // Step 4. Move the entries, and update the parent pointers of the moved pages.
    for (int i = 0; i < nRight; ++i) {
    ...
    // Step 5. Set the new children page of `midEntry`.
    midEntry.setLeftChild(page.pid);
    midEntry.setRightChild(newPage.pid);
    // Step 6. Fetch the parent page by calling `getParentWithEmptySlots()`. Note that the
    //         recursive split happens in this method.
    BTreeInternalPage parentPage = getParentWithEmptySlots(tid, dirtypages, page.getParentId(),
        midEntry.getKey());
    // Step 7. Update the parent pointers.
    updateParentPointer(tid, dirtypages, parentPage.pid, page.pid);
    updateParentPointer(tid, dirtypages, parentPage.pid, newPage.pid);
    // Step 8. Insert the new entry to the parent.
  ...
  private BTreeInternalPage getParentWithEmptySlots(TransactionId tid,
      HashMap<PageId, Page> dirtypages, BTreePageId parentId, Field field)
      throws DbException, IOException, TransactionAbortedException {
   ...
   */
  Page getPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreePageId pid,
      Permissions perm) throws DbException, TransactionAbortedException {
   ...
   * @see #handleMinOccupancyLeafPage(TransactionId, HashMap, BTreeLeafPage, BTreeInternalPage,
   * BTreeEntry, BTreeEntry)
   * @see #handleMinOccupancyInternalPage(TransactionId, HashMap, BTreeInternalPage,
   * BTreeInternalPage, BTreeEntry, BTreeEntry)
   */
  private void handleMinOccupancyPage(TransactionId tid, HashMap<PageId, Page> dirtypages,
   ...
   * @see #mergeLeafPages(TransactionId, HashMap, BTreeLeafPage, BTreeLeafPage, BTreeInternalPage,
   * BTreeEntry)
   * @see #stealFromLeafPage(BTreeLeafPage, BTreeLeafPage, BTreeInternalPage, BTreeEntry, boolean)
   */
  private void handleMinOccupancyLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages,
      BTreeLeafPage page, BTreeInternalPage parent, BTreeEntry leftEntry, BTreeEntry rightEntry)
      throws DbException, IOException, TransactionAbortedException {
  ...
  protected void stealFromLeafPage(BTreeLeafPage page, BTreeLeafPage sibling,
    ...
    int nThisPage = page.getNumTuples();
    int nSiblingPage = sibling.getNumTuples();
    assert nThisPage + 1 < nSiblingPage;
    // Step 1. Fetch the tuples to be moved.
    final int nMove = (nThisPage + nSiblingPage) / 2 - nThisPage;
    assert nMove >= 1;
    Tuple[] tuples = new Tuple[nMove];
    Iterator<Tuple> it = isRightSibling ? sibling.iterator() : sibling.reverseIterator();
    for (int i = 0; i < nMove; ++i) {
      assert it.hasNext();
      tuples[i] = it.next();
    }
    // Step 2. Move the tuples.
    for (int i = 0; i < nMove; ++i) {
      sibling.deleteTuple(tuples[i]);
      page.insertTuple(tuples[i]);
    }
    // Step 3. Update the parent entry.
    if (isRightSibling) {
   ...
   * @param leftEntry  - the entry in the parent pointing to the given page and its left-sibling
   * @param rightEntry - the entry in the parent pointing to the given page and its right-sibling
   * @see #mergeInternalPages(TransactionId, HashMap, BTreeInternalPage, BTreeInternalPage,
  ...
  protected void stealFromLeftInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages,
    ...
    int nRight = page.getNumEntries();
    int nLeft = leftSibling.getNumEntries();
    assert nRight + 1 < nLeft;
    // Step 1. Fetch the entries to be moved.
    final int nMove = (nRight + nLeft) / 2 - nRight;
    assert nMove >= 1;
    BTreeEntry[] entries = new BTreeEntry[nMove];
    Iterator<BTreeEntry> it = leftSibling.reverseIterator();
    for (int i = 0; i < nMove; ++i) {
      assert it.hasNext();
      entries[i] = it.next();
    }
    // Step 2. Mark three pages as dirty.
    ...
    for (int i = 0; i < nMove; ++i) {
      ...
      // Step 3.4. Update the parent pointer of the moved page.
      updateParentPointer(tid, dirtypages, page.pid, entries[i].getLeftChild());
      // Step 3.5. Update the key of the parent entry.
  ...
  protected void stealFromRightInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages,
    ...
    int nLeft = page.getNumEntries();
    int nRight = rightSibling.getNumEntries();
    assert nLeft + 1 < nRight;
    // Step 1. Fetch the entries to be moved.
    final int nMove = (nLeft + nRight) / 2 - nLeft;
    assert nMove >= 1;
    BTreeEntry[] entries = new BTreeEntry[nMove];
  ...
  protected void mergeLeafPages(TransactionId tid, HashMap<PageId, Page> dirtypages,
    ...
    // Step 1. Fetch the tuples to be moved.
    final int nMoved = rightPage.getNumTuples();
    Tuple[] tuples = new Tuple[nMoved];
    ...
    // Step 3. Remove the parent entry. Note that the recursive stealing or merging might happen
    //         in this method.
    ...
    // Step 6. Set the right page as empty.
  ...
  protected void mergeInternalPages(TransactionId tid, HashMap<PageId, Page> dirtypages,
    ...
    // Step 1. Fetch the entries to be moved.
    final int nMove = rightPage.getNumEntries();
    BTreeEntry[] entries = new BTreeEntry[nMove];
    ...
    dirtypages.put(leftPage.pid, leftPage);
    dirtypages.put(rightPage.pid, rightPage);
    // Step 3. Delete the parent entry in the parent page. Note that the recursive stealing or
    //         merging might happen in this method.
    deleteParentEntry(tid, dirtypages, leftPage, parent, parentEntry);
    // Step 4. Move the entries. Note that we first pulling down the parent entry.
    ...
    // Step 5. Update the parent pointers of the moved pages.
    updateParentPointers(tid, dirtypages, leftPage);
    // Step 6. Mark the right page as empty.
    setEmptyPage(tid, dirtypages, rightPage.pid.pageNumber());
  }
...
class BTreeFileIterator extends AbstractDbFileIterator {
  ...
  public void open() throws DbException, TransactionAbortedException {
    BTreeRootPtrPage rootPtr = (BTreeRootPtrPage) Database.getBufferPool()
        .getPage(tid, BTreeRootPtrPage.getId(f.getId()), Permissions.READ_ONLY);
    ...
    while (it != null) {
      ...
      while (it.hasNext()) {
        ...
        if (t.getField(f.keyField()).compare(ipred.getOp(), ipred.getField())) {
          ...
          // hit the end
          return null;
        } else if (ipred.getOp() == Op.EQUALS && t.getField(f.keyField())
            .compare(Op.GREATER_THAN, ipred.getField())) {
