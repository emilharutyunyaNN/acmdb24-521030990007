...
public class BTreeInternalPage extends BTreePage {
  ...
  public int getMaxEntries() {
    ...
    int entriesPerPage =
        (BufferPool.getPageSize() * 8 - extraBits) / bitsPerEntryIncludingHeader; //round down
  ...
  private void deleteEntry(BTreeEntry e, boolean deleteRightChild) throws DbException {
    ...
    if ((rid.getPageId().pageNumber() != pid.pageNumber()) || (rid.getPageId().getTableId() != pid
        .getTableId())) {
  ...
  public void insertEntry(BTreeEntry e) throws DbException {
    ...
    for (int i = 0; i < numSlots; i++) {
      if (isSlotUsed(i)) {
        if (children[i] == e.getLeftChild().pageNumber() || children[i] == e.getRightChild()
            .pageNumber()) {
          if (i > 0 && keys[i].compare(Op.GREATER_THAN, e.getKey())) {
